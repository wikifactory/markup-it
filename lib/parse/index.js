...
/**
    ...
    @return {List<Token>}
*/
function parseAsInlineTokens(syntax, text, parent, ctx) {
    var inlineRulesSet = syntax.getInlineRulesSet();
    var inlineRules = inlineRulesSet.getRules();

    // Parse block tokens
    var tokens = lex.inline(inlineRules, text, parent, ctx);
...
/**
    ...
    @return {List<Token>}
*/
function parseAsBlockTokens(syntax, text, parent, ctx) {
    text = cleanup(text);
    ...
    // Parse block tokens
    var tokens = lex.block(blockRules, text, parent, ctx);
...
function parseInnerToken(syntax, token, ctx) {
    ...
    // Build syntax
    syntax = syntax.filterRules(function(innerRule) {
        return rule.getOption('parse.' + innerRule.getType(), true);
    });
    ...
    if (tokens.size === 0) {
        if (parseMode === 'block') {
            tokens = parseAsBlockTokens(syntax, token.getText(), token, ctx);
        } else {
            tokens = parseAsInlineTokens(syntax, token.getText(), token, ctx);
        }
...
function parseAsContent(syntax, text, ctx) {
    ...
    // Parse inline content for each token
    var tokens = parseAsBlockTokens(syntax, text, null, ctx);
