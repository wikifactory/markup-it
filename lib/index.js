    ...
    while(text) {
        for (var key in this.rules.blocks) {
            ...
            if (rule.inline !== false) {
                ...
                for (var e in block.entityRanges) {
                    block.entityRanges[e].key = 'entity' + entityId;
                    entityMap[block.entityRanges[e].key] = block.entityRanges[e].entity;
                    delete block.entityRanges[e].entity
...
DraftText.prototype.applyInlineRanges = function applyInlineRanges(text, inlineStyleRanges, entityRanges, entityMap) {
    ...
    var inlineElements = inlineStyleRanges.concat(entityRanges);
    ...
    for (var i in inlineStyleRanges) {
        ...
        // Is entity?
        entity = range.key? entityMap[range.key] : null;
